...
TAG=spurin/${PWD##*/}:$(git branch | grep '*' | awk {'print $2'} | sed 's/main/latest/g' | sed 's/master/latest/g')

if [ $# == 0 ]; then
  echo build.sh [options]
  echo Where options are -l = local, -c = crossbuild, -cr = crossbuild using a remote host
fi
...
while [[ $# -gt 0 ]]; do
   ...
   -c | --crossbuild)
      ...
      docker buildx build --platform linux/amd64,linux/arm64/v8 -t $TAG . --push
      exit
      ;;
   -cr | --crossbuildremote)
